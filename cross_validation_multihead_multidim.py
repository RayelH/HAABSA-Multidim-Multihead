...
import lcrModelAlt_hierarchical_v4_multihead_multidim_cross
...
def cross_val(number_of_folds=10, compute_folds=False, multidim=False, multihead=False, multidimmultihead=False,
    ...
    if multidim:
        ...
        for i in range(number_of_folds):
            min_training_loss, max_training_accuracy, min_test_loss, max_test_accuracy, training_losses, train_accuracies, test_losses, test_accuracies = lcrModelAlt_hierarchical_v4_multihead_multidim_cross.main(BASE_train + str(i) + '.txt', BASE_val + str(i) + '.txt',
        ...
        with open("cross_results_" + str(FLAGS.year) + "/multihead_multidim_new" + str(FLAGS.year) + '.txt', 'a') as result:
            result.write('Config: l_2 = {}, learning rate = {}, number_epochs = {}, dropout keep probability = {}, number_of_heads = {} \n'.format(l2, learning_rate, number_epochs, keep_prob, number_of_heads))
            ...
            for k in range(len(number_epochs)):
                for q in range(len(head_sizes)):
                    ...
                    cross_val(number_of_folds=5, compute_folds=compute_folds, multidim=True,
                              learning_rate=learning_rates[i], l2=l2s[j], keep_prob=keep_prob,
                              number_of_heads=head_sizes[q],
