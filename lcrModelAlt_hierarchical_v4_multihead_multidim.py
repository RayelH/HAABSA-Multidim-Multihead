...
def lcr_rot(input_fw, input_bw, sen_len_fw, sen_len_bw, target, sen_len_tr, keep_prob1, keep_prob2, l2, _id='all', number_of_heads=FLAGS.heads):
    ...
    return prob, hiddens_l_new, hiddens_r_new, hiddens_t_l_new, hiddens_t_r_new
...
def main(train_path, test_path, accuracyOnt, trainAccuracyOnt, test_size, remaining_size, learning_rate=FLAGS.learning_rate, keep_prob=FLAGS.keep_prob1, momentum=0.9, l2=FLAGS.l2_reg, number_of_heads=FLAGS.heads):
    ...
    with tf.device('/gpu:1'):
        ...
        prob, alpha_fw, alpha_bw, alpha_t_l, alpha_t_r = lcr_rot(inputs_fw, inputs_bw, sen_len, sen_len_bw, target,
                                                                 tar_len, keep_prob1, keep_prob2, l2, 'all', number_of_heads)
    ...
    with tf.Session(config=config) as sess:
        ...
        for i in range(FLAGS.n_iter):
            ...
            for train, numtrain in get_batch_data(tr_x, tr_sen_len, tr_x_bw, tr_sen_len_bw, tr_y, tr_target_word,
                ...
                _, step, _trainacc, _training_loss = sess.run([optimizer, global_step, acc_num, loss],
                                                       feed_dict=train)
        ...
        # fp = open(FLAGS.prob_file + '_multihead_multidim' + str(FLAGS.year), 'w')
        # for y1, y2, item in zip(max_ty, max_py, max_prob):
        #     fp.write(str(y1) + ' ' + str(y2) + ' ' + ' '.join([str(it) for it in item]) + '\n')
        #
        # with open(FLAGS.prob_file + '_fw_multihead_multidim' + str(FLAGS.year), 'w') as outfile:
        #     np.savetxt(outfile, max_fw)
        #
        # with open(FLAGS.prob_file + '_bw_multihead_multidim' + str(FLAGS.year), 'w') as outfile:
        #     np.savetxt(outfile, max_bw)
        #
        # with open(FLAGS.prob_file + '_tl_multihead_multidim' + str(FLAGS.year), 'w') as outfile:
        #     np.savetxt(outfile, max_tl)
        #
        # with open(FLAGS.prob_file + '_tr_multihead_multidim' + str(FLAGS.year), 'w') as outfile:
