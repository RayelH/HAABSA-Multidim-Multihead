...
def lcr_rot(input_fw, input_bw, sen_len_fw, sen_len_bw, target, sen_len_tr, keep_prob1, keep_prob2, l2, _id='all'):
    ...
    # attention left
    att_l = multidimensional_attention_layer(hiddens_l, pool_t, sen_len_fw, 2 * FLAGS.n_hidden, FLAGS.l2_reg, FLAGS.random_base, 'tl')
    ...
    # attention right
    att_r = multidimensional_attention_layer(hiddens_r, pool_t, sen_len_bw, 2 * FLAGS.n_hidden, FLAGS.l2_reg, FLAGS.random_base, 'tr')
    ...
    att_t_l = multidimensional_attention_layer(hiddens_t, outputs_t_l, sen_len_tr, 2 * FLAGS.n_hidden, FLAGS.l2_reg, FLAGS.random_base, 'l')
    ...
    att_t_r = multidimensional_attention_layer(hiddens_t, outputs_t_r, sen_len_tr, 2 * FLAGS.n_hidden, FLAGS.l2_reg, FLAGS.random_base, 'r')
    ...
    for i in range(2):
        # attention target
        att_l = multidimensional_attention_layer(hiddens_l, outputs_l, sen_len_fw, 2 * FLAGS.n_hidden, FLAGS.l2_reg, FLAGS.random_base, 'tl'+str(i))
        outputs_t_l_init = tf.transpose(tf.reduce_sum((tf.multiply(att_l, tf.transpose(hiddens_l, perm=[0, 2, 1]))), reduction_indices=-1, keep_dims=True) + 1e-9,[0,2,1])
        outputs_t_l = tf.squeeze(outputs_t_l_init)

        att_r = multidimensional_attention_layer(hiddens_r, outputs_r, sen_len_bw, 2 * FLAGS.n_hidden, FLAGS.l2_reg, FLAGS.random_base, 'tr'+str(i))
        outputs_t_r_init = tf.transpose(tf.reduce_sum((tf.multiply(att_r, tf.transpose(hiddens_r, perm=[0, 2, 1]))), reduction_indices=-1, keep_dims=True) + 1e-9,[0,2,1])
        outputs_t_r = tf.squeeze(outputs_t_r_init)

        # attention left
        att_t_l = multidimensional_attention_layer(hiddens_t, outputs_t_l, sen_len_tr, 2 * FLAGS.n_hidden, FLAGS.l2_reg, FLAGS.random_base, 'l'+str(i))
        outputs_l_init = tf.transpose(tf.reduce_sum((tf.multiply(att_t_l, tf.transpose(hiddens_t, perm=[0, 2, 1]))), reduction_indices=-1, keep_dims=True) + 1e-9,[0,2,1])
        outputs_l = tf.squeeze(outputs_l_init)

        # attention right
        att_t_r = multidimensional_attention_layer(hiddens_t, outputs_t_r, sen_len_tr, 2 * FLAGS.n_hidden, FLAGS.l2_reg, FLAGS.random_base, 'r'+str(i))
        outputs_r_init = tf.transpose(tf.reduce_sum((tf.multiply(att_t_r, tf.transpose(hiddens_t, perm=[0, 2, 1]))), reduction_indices=-1, keep_dims=True) + 1e-9,[0,2,1])
        outputs_r = tf.squeeze(outputs_r_init)
    ...
    prob = softmax_layer(outputs_fin, 8 * FLAGS.n_hidden, FLAGS.random_base, keep_prob2, FLAGS.l2_reg, FLAGS.n_class)
...
def main(train_path, test_path, accuracyOnt, trainAccuracyOnt, test_size, remaining_size, learning_rate=FLAGS.learning_rate, keep_prob=FLAGS.keep_prob1, momentum=0.9, l2=FLAGS.l2_reg):
    ...
    with tf.Session(config=config) as sess:
        ...
        average_max_fw = np.mean(max_fw, axis=1)
        stdev_max_fw = np.std(max_fw, axis=1)
        average_max_bw = np.mean(max_bw, axis=1)
        stdev_max_bw = np.std(max_bw, axis=1)
        average_max_tl = np.mean(max_tl, axis=1)
        stdev_max_tl = np.std(max_tl, axis=1)
        average_max_tr = np.mean(max_tr, axis=1)
        stdev_max_tr = np.std(max_tr, axis=1)



        fp = open(FLAGS.prob_file + '_fw_multidim_average' + str(FLAGS.year), 'w')
        for y1, y2, ws in zip(max_ty, max_py, average_max_fw):
            fp.write(str(y1) + ' ' + str(y2) + ' ' + ' '.join([str(w) for w in ws]) + '\n')
        fp.write(str(stdev_max_fw))

        fp = open(FLAGS.prob_file + '_bw_multidim_average' + str(FLAGS.year), 'w')
        for y1, y2, ws in zip(max_ty, max_py, average_max_bw):
            fp.write(str(y1) + ' ' + str(y2) + ' ' + ' '.join([str(w) for w in ws]) + '\n')
        fp.write(str(stdev_max_bw))

        fp = open(FLAGS.prob_file + '_tl_multidim_average' + str(FLAGS.year), 'w')
        for y1, y2, ws in zip(max_ty, max_py, average_max_tl):
            fp.write(str(y1) + ' ' + str(y2) + ' ' + ' '.join([str(w) for w in ws]) + '\n')
        fp.write(str(stdev_max_tl))

        fp = open(FLAGS.prob_file + '_tr_multidim_average' + str(FLAGS.year), 'w')
        for y1, y2, ws in zip(max_ty, max_py, average_max_tr):
            fp.write(str(y1) + ' ' + str(y2) + ' ' + ' '.join([str(w) for w in ws]) + '\n')
        fp.write(str(stdev_max_tr))
