...
def cross_val(number_of_folds=10, compute_folds=False, multidim=False, multihead=False, multidimmultihead=False,
    ...
    if multidim:
        ...
        for i in range(number_of_folds):
            min_training_loss, max_training_accuracy, min_test_loss, max_test_accuracy, training_losses, train_accuracies, test_losses, test_accuracies = lcrModelAlt_hierarchical_v4_multidim_cross.main(BASE_train + str(i) + '.txt', BASE_val + str(i) + '.txt',
                                                         learning_rate=learning_rate, keep_prob=keep_prob, beta=beta, l2=l2, number_epochs=number_epochs)
        ...
        with open("cross_results_" + str(FLAGS.year) + "/multidim_new" + str(FLAGS.year) + '.txt', 'a') as result:
            result.write('Config: l_2 = {}, learning rate = {}, number_epochs = {}, dropout keep probability = {} \n'.format(l2, learning_rate, number_epochs, keep_prob))
            result.write('Max Validation Accuracy = {}, at Epoch {} \n'.format(avg_max_test_accuracy,
                                                                               index_avg_max_test_accuracy))
    ...
    for i in range(len(learning_rates)):
        for j in range(len(l2s)):
            for k in range(len(number_epochs)):
                count += 1
                print('learning_rate = {}, l2 = {}, number_epochs = {}, keep_prob = {}, count = {}'.format(learning_rates[i], l2s[j], number_epochs[k], keep_prob, count))
                cross_val(number_of_folds=5, compute_folds=compute_folds, multidim=True, learning_rate=learning_rates[i], l2=l2s[j], keep_prob=keep_prob, number_epochs=number_epochs[k])
                compute_folds = False
