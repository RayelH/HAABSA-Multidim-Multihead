...
def softmax_with_len_multidimensional(inputs, length, max_len, n_hidden, batch_size):
    inputs = tf.cast(inputs, tf.float32)  # batch_size * n_hidden * max_sen_len

    inputs = tf.exp(inputs) # batch_size * n_hidden * max_sen_len

    if length is not None:
        length = tf.reshape(length, [-1])  # puts all sentence lengths in one dimensional tensor
        mask = tf.reshape(tf.cast(tf.sequence_mask(length, max_len), tf.float32), [batch_size, 1, max_len])
        inputs *= mask
...
def multidimensional_attention_layer(inputs, attend, length, n_hidden, l2_reg, random_base, layer_id=1):
    """
    ...
    # inputs: (batch_size*max_sen_len) * 600 = alle sentence matrices onder elkaar, elke rij is een word embedding
    inputs = tf.reshape(inputs, [-1,
                                 n_hidden])  # zet alle woorden van alle zinnen onder elkaar in 2d matrix dim: (batch_size*max_sen_len)*600

    # tmp: batch_size * max_sen_len * 600
    tmp = tf.reshape(tf.matmul(inputs, w1), [-1, max_len, n_hidden])

    # attend: batch_size * 600 * 1
    attend = tf.expand_dims(attend, 2)

    # batch_size * 1 * max_sen_len
    tmp = tf.reshape(tf.tanh(tf.matmul(tmp, attend) + b), [batch_size, 1, max_len])

    tmp2 = tf.reshape(tf.transpose(tmp,[1,0,2]),[1,-1])

    # batch_size * n_hidden * max_sen_len
    output = tf.transpose(tf.reshape(tf.matmul(w0,tmp2),[n_hidden,-1, max_len]), [1,0,2])

    return softmax_with_len_multidimensional(output, length, max_len, n_hidden, batch_size)
...
def multidimensional_attention_layer_old(inputs, attend, length, n_hidden, l2_reg, random_base, layer_id=1):
    """
       :param inputs: batch * max_len * n_hidden
       :param attend: batch * n_hidden
       :param length:
...
def multidimensional_attention_layer(inputs, attend, length, n_hidden, l2_reg, random_base, layer_id=1):
    ...
    # inputs: (batch_size*max_sen_len) * 600 = alle sentence matrices onder elkaar, elke rij is een word embedding
    inputs = tf.reshape(inputs, [-1, n_hidden])  # zet alle woorden van alle zinnen onder elkaar in 2d matrix dim: (batch_size*max_sen_len)*600

    # tmp: batch_size * max_sen_len * 600
    tmp = tf.reshape(tf.matmul(inputs, w1), [-1, max_len, n_hidden])

    # attend: batch_size * 1 * n_hidden
    attend = tf.expand_dims(attend, 1)

    # batch_size * n_hidden
    reshaped_attend = tf.reshape(attend, [-1, n_hidden])

    # batch_size * 1 * n_hidden
    weighted_representation = tf.reshape(tf.matmul(reshaped_attend, w2), [-1, 1, n_hidden])

    # batch_size * max_len * n_hidden
    attention = tmp + weighted_representation

    if length is not None:
        length = tf.reshape(length, [-1])  # puts all sentence lengths in one dimensional tensor
        mask = tf.reshape(tf.cast(tf.sequence_mask(length, max_len), tf.float32), [batch_size, max_len, 1])
        attention_masked = attention * mask

    # batch_size * max_len * n_hidden
    activated_attention = tf.tanh(attention_masked + b)

    # batch_size * max_len * n_hidden
    scores = tf.reshape(tf.matmul(tf.reshape(activated_attention, [-1, n_hidden]), w3), [-1, max_len, n_hidden])

    # batch_size * n_hidden * max_sen_len
    output = tf.transpose(scores, [0,2,1])
